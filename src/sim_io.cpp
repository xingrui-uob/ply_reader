...
pcl::simulation::SimExample::SimExample(int argc, char **argv, std::string intr_path, std::string ply_file)
{

    initializeGL(argc, argv);

    std::ifstream file(intr_path);
    float w, h, fx, fy, cx, cy;
    file >> w >> h >> fx >> fy >> cx >> cy;
    height_ = (int)h;
    width_ = (int)w;
    file.close();
    ...
    // rl_ = RangeLikelihood::Ptr(new RangeLikelihood(1, 1, height_, width_, scene_));
    camera_->setParameters(width_, height_, fx, fy, cx, cy, 0.7, 20.0);

    // Actually corresponds to default parameters:
    rl_->setCameraIntrinsicsParameters(width_, height_, fx, fy, cx, cy);
    rl_->setComputeOnCPU(false);
    rl_->setSumOnCPU(true);
    rl_->setUseColor(true);

    // 2. read mesh and setup model:
    // std::cout << "About to read: " << ply_file << std::endl;
    pcl::PolygonMesh mesh; // (new pcl::PolygonMesh);
    pcl::io::loadPolygonFile(ply_file, mesh);
    ...
    // std::cout << "Just read " << ply_file << std::endl;
    ...
    // camera_->set(0, 0.0, 0.0, 3.0, 4.0, 3.0);
...
void pcl::simulation::SimExample::initializeGL(int argc, char **argv)
{
    glutInit(&argc, argv);
...
void pcl::simulation::SimExample::doSim(Eigen::Isometry3d pose_in)
{
...
void pcl::simulation::SimExample::write_score_image(const float *score_buffer,
                                                    std::string fname)
{
...
void pcl::simulation::SimExample::write_depth_image(const float *depth_buffer,
                                                    std::string fname)
{
    int npixels = rl_->getWidth() * rl_->getHeight();
...
void pcl::simulation::SimExample::write_depth_image_uint(const float *depth_buffer,
                                                         std::string fname)
{
    int npixels = rl_->getWidth() * rl_->getHeight();
...
void pcl::simulation::SimExample::write_rgb_image(const std::uint8_t *rgb_buffer,
                                                  std::string fname)
{
    int npixels = rl_->getWidth() * rl_->getHeight();
